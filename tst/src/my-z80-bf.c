...
static void dump_state(char * what, uint64_t state);

static uint32_t my_z80_bf_F(struct my_z80_bf * bf, uint32_t x);

static uint64_t my_z80_bf_encrypt(struct my_z80_bf * bf, uint64_t plaintext );
static uint64_t my_z80_bf_decrypt(struct my_z80_bf * bf, uint64_t ciphertext);
...
{
	static const char name[]="own Z80 blowfish";
	...
	ciph->ciph_init   = &my_z80_bf_ciph_init;
	ciph->ciph_setkey = &my_z80_bf_ciph_setkey;
	ciph->ciph_enc    = &my_z80_bf_ciph_enc;
	ciph->ciph_dec    = &my_z80_bf_ciph_dec;
	ciph->ciph_deinit = &my_z80_bf_ciph_deinit;
...
{ // init blowfish cipher structure
	...
	struct my_z80_bf * bf = malloc(sizeof(struct my_z80_bf));
	...
	if( !bf )
	{
		fprintf(stderr,"%s: %d, %s: can't allocate memory for struct my_z80_bf!\n",__FILE__,__LINE__,__func__);
		exit(1);
	}


	// load Z80 binary
	z80 = z80_init("z80/bf_test.bin");

	bf->z80 = z80;

	unsigned clocks = z80_execute(z80,Z80_MAX_CLOCKS,Z80_BF_INIT);
	if( !clocks )
	{
		fprintf(stderr,"%s: %d, %s: Z80 bf init didn't return after %d clocks!\n",__FILE__,__LINE__,__FUNCTION__,Z80_MAX_CLOCKS);
		exit(1);
	}
	printf("BF: %d clocks to init (%s)\n",clocks,__func__);
...
{
	struct my_z80_bf * bf = (struct my_z80_bf *)ciph->ciph_specific_data;



	if( 1>key_size || key_size>56 )
	{
		fprintf(stderr,"%s: %d, %s: blowfish accepts only key lengths 1..56 bytes! Requested key length=%ld!\n",__FILE__,__LINE__,__FUNCTION__,key_size);
		exit(1);
	}
	...
	{
		z80_wrbyte(bf->z80,Z80_BF_INKEY_ADDR+i,key[i]);
	}
	z80_wrbyte(bf->z80,Z80_BF_INKEY_LEN_ADDR,key_size);
	// run Z80 setkey
	unsigned clocks = z80_execute(bf->z80,Z80_MAX_CLOCKS,Z80_BF_SETKEY);
	if( !clocks )
	{
		fprintf(stderr,"%s: %d, %s: Z80 blowfish setkey didn't return after %d clocks!\n",__FILE__,__LINE__,__FUNCTION__,Z80_MAX_CLOCKS);
		fprintf(stderr,"pc=%04x\n",bf->z80->z80.PC);
		exit(1);
	}
	printf("BF: %d clocks to setkey (%s)\n",clocks,__func__);

//printf("Z80 bf state: ");
//for(int i=0;i<8;i++)
// printf("%02x",z80_mem[Z80_BF_STATE_ADDR+i]);
//printf("\n");
	...
	{
		seed = my_z80_bf_encrypt(bf,seed);
...
/////////////////////////////////////////////
	...
	{
		uint8_t z80 = z80_rdbyte(bf->z80,Z80_BF_KEYS_ADDR+i);
		uint8_t byte = bf->p[i>>2]>>(24-(i&3)*8);

		if( z80!=byte )
		{
			fprintf(stderr,"%s: key check: at %d, z80=%02x, tbl=%02x\n",__func__,i,z80,byte);
			exit(1);
		}
	}
/////////////////////////////////////////////
...
/////////////////////////////////////////////
	// check S-boxes of Z80

	uint32_t * ptr[4] = {bf->s0,bf->s1,bf->s2,bf->s3};
	uint16_t ptr_z80[4][4] =
	{
		{Z80_BF_S0_B0_ADDR, Z80_BF_S0_B1_ADDR, Z80_BF_S0_B2_ADDR, Z80_BF_S0_B3_ADDR},
		{Z80_BF_S1_B0_ADDR, Z80_BF_S1_B1_ADDR, Z80_BF_S1_B2_ADDR, Z80_BF_S1_B3_ADDR},
		{Z80_BF_S2_B0_ADDR, Z80_BF_S2_B1_ADDR, Z80_BF_S2_B2_ADDR, Z80_BF_S2_B3_ADDR},
		{Z80_BF_S3_B0_ADDR, Z80_BF_S3_B1_ADDR, Z80_BF_S3_B2_ADDR, Z80_BF_S3_B3_ADDR}
	};
	for(int i=0;i<4;i++)   // table
	for(int j=0;j<256;j++) // table pos
	for(int k=0;k<4;k++)   // byte pos
	{
		uint8_t byte = ptr[i][j]>>(24-(k*8));
		uint8_t z80  = z80_rdbyte(bf->z80,ptr_z80[i][k]+j);

		if( byte!=z80 )
		{
			fprintf(stderr,"%s: tbl %d, pos %d, byte %d: z80=%02x, mustbe=%02x\n",__func__,i,j,k,z80,byte);
			exit(1);
		}
	}
/////////////////////////////////////////////
...
int    my_z80_bf_ciph_enc(struct ciph_iface * ciph, uint8_t * plain, uint8_t * cipher, size_t size)
{
	...
	for(size_t i=0;i<size;i+=8)
	{
		...
		for(int j=0;j<8;j++)
		{
			z80_wrbyte(bf->z80,Z80_BF_INDATA_ADDR+j,plain[i+j]);
		}
		// run Z80 encrypt
		unsigned clocks = z80_execute(bf->z80, Z80_MAX_CLOCKS, Z80_BF_ENCRYPT);
		if( !clocks )
		{
			fprintf(stderr,"%s: %d, %s: Z80 blowfish encrypt didn't return after %d clocks!\n",__FILE__,__LINE__,__FUNCTION__,Z80_MAX_CLOCKS);
			exit(1);
		}
		printf("BF: %d clocks to encrypt (%s)\n",clocks,__func__);
		...
		data = my_z80_bf_encrypt(bf,data);

		for(size_t j=0;j<8;j++)
		{
			cipher[i+j] = data>>(56-8*j);
		}
//dump_state("Z80", *((uint64_t *)&z80_mem[Z80_BF_STATE_ADDR]));

		////////////////////////////////////////////////
		// check Z80 encryption
		for(int j=0;j<8;j++)
		{
			uint8_t byte = cipher[i+j];
			uint8_t z80 = z80_rdbyte(bf->z80,Z80_BF_OUTDATA_ADDR+j);
			if( byte!=z80 )
			{
				fprintf(stderr,"%s: encryption mismatch: pos %d: z80=%02x, mustbe=%02x\n",__func__,j,z80,byte);
				exit(1);
			}
		}
		////////////////////////////////////////////////
...
int    my_z80_bf_ciph_dec(struct ciph_iface * ciph, uint8_t * cipher, uint8_t * plain, size_t size)
{
	...
	for(size_t i=0;i<size;i+=8)
	{
		...
		data = my_z80_bf_decrypt(((struct my_z80_bf *)ciph->ciph_specific_data),data);
...
static uint32_t my_z80_bf_F(struct my_z80_bf * bf, uint32_t x)
{
	...
	uint32_t q = ((bf->s0[a] + bf->s1[b]) ^ bf->s2[c]) + bf->s3[d];
//printf("F=%08x\n",q);
	return q;
}
...
//printf("round %d, ",i);
//dump_state("state",(((uint64_t)l)<<32) | ((uint64_t)r));
	}
...
//dump_state("final",(((uint64_t)l)<<32) | ((uint64_t)r));
